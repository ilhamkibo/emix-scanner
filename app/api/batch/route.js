...
export async function POST(req) {
  try {
    ...
    const { batchId, packIndex } = body;

    // Validasi input
    if (!batchId) {
      return NextResponse.json(
        { status: 400, message: "Bad Request: batchId is required" },
        { status: 400 }
      );
    }

    console.log(`${process.env.API_URL}/pack/generate/${batchId}`);

    // Buat permintaan ke API eksternal
    const res = await fetch(`${process.env.API_URL}/pack/generate/${batchId}`, {
      method: "POST",
      headers: {
